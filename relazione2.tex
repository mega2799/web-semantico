...
\begin{itemize}
    ...
    \item \textbf{Riusabilità e estensibilità} - Garantire che l'ontologia sia facilmente integrabile con altre ontologie del dominio food \& beverage e aggiornabile con nuovi stili, ingredienti e tecniche di produzione.
\end{itemize}
...
L'ontologia include inoltre informazioni su microbirrifici, brewery contract, e craft brewery, fornendo un vocabolario completo per descrivere l'intera filiera produttiva. La documentazione è disponibile in inglese e francese, garantendo accessibilità a una vasta comunità internazionale.
...
\begin{itemize}
    ...
    \item \textbf{Origine geografica}: paese e regione di coltivazione, effetti del terroir
    \item \textbf{Usi tipici}: bittering, aroma, dual-purpose
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Processo di produzione}: Munich Malt, Pilsner Malt, Caramel Malt, Chocolate Malt, Black Malt, Roasted Malt
\end{enumerate}
...
Le Data Properties permettono di associare valori letterali alle istanze:

\paragraph{Proprietà della Recipe:}
Parametri identificativi e descrittivi (\texttt{recipeName}, \texttt{recipeDescription}, \texttt{author}, \texttt{creationDate}), parametri tecnici fondamentali (\texttt{estimatedABV}, \texttt{estimatedIBU}, \texttt{estimatedEBC}, \texttt{originalGravity}, \texttt{finalGravity}), parametri di produzione (\texttt{batchSize}, \texttt{totalBrewingTime}, \texttt{mashingTime}, \texttt{boilingTime}, \texttt{fermentationDuration}, \texttt{conditioningDuration}), e classificazioni (\texttt{difficultyLevel}, \texttt{servingTemperature}, \texttt{isVegan}, \texttt{isGlutenFree}).

\paragraph{Proprietà degli Ingredient:}
Attributi comuni (\texttt{ingredientName}, \texttt{quantity}, \texttt{unit}), proprietà specifiche dei luppoli (\texttt{hopName}, \texttt{alphaAcidPercentage}, \texttt{additionTime}), dei malti (\texttt{maltName}, \texttt{ebcColor}), dei cereali (\texttt{cerealName}, \texttt{glutenContent}), e dei lieviti (\texttt{yeastName}, \texttt{optimalTemperature}, \texttt{minAttenuation}, \texttt{maxAttenuation}).

\paragraph{Proprietà dei BrewingStep:}
\texttt{stepName}, \texttt{stepOrder}, \texttt{stepDuration}, \texttt{stepTemperature}, \texttt{stepInstructions} permettono di descrivere completamente ogni fase del processo produttivo.

\paragraph{Proprietà dei BeerStyle:}
\texttt{styleName}, \texttt{styleDescription}, range di parametri tecnici (\texttt{minABV}, \texttt{maxABV}, \texttt{minIBU}, \texttt{maxIBU}) che definiscono i confini di ogni stile secondo gli standard internazionali.
...
\begin{verbatim}
DisjointClasses(Hop, Malt, Cereal, Yeast)    TO_IMPLEMENT
\end{verbatim}
...
Una ricetta non può appartenere a più livelli di difficoltà contemporaneamente.

\begin{verbatim}
DisjointClasses(AleStyle, LagerStyle, LambicStyle)  TO_IMPLEMENT
\end{verbatim}
...
\subsubsection{Livelli di difficoltà}

\paragraph{Problema:}
Classificare le ricette per difficoltà in modo oggettivo e utilizzabile.

\paragraph{Alternative considerate:}

\textbf{Valutazione soggettiva}: Lasciare all'autore la libertà di assegnare un livello di difficoltà arbitrario.

\textbf{Calcolo automatico}: Definire regole SWRL che calcolano la difficoltà basandosi su parametri oggettivi (numero di ingredienti, complessità delle fasi, parametri tecnici critici).

\textbf{Sistema misto}: Difficoltà dichiarata + validazione automatica.

\paragraph{Scelta finale:}
Inizialmente si utilizza una \textbf{classificazione manuale} con valori standardizzati (beginner, intermediate, advanced, expert), con l'obiettivo futuro di implementare regole SWRL per suggerire automaticamente il livello di difficoltà basandosi su metriche oggettive come:
\begin{itemize}
    \item Numero di ingredienti diversi
    \item Numero di fasi di produzione
    \item Complessità dei parametri (controllo preciso di temperatura, pH, timing)
    \item Necessità di attrezzatura avanzata
    \item Durata totale del processo
\end{itemize}
...
\subsubsection{Considerazioni sulle performance}

Tutte le scelte di modellazione sono state valutate anche considerando le performance di reasoning e query:
\begin{itemize}
    \item Le restrizioni OWL sono state limitate a quelle strettamente necessarie per evitare reasoning eccessivamente costoso
    \item Le proprietà transitive sono usate con parsimonia (solo per \texttt{isSimilarTo})
    \item La profondità della gerarchia delle classi è mantenuta ridotta (massimo 2-3 livelli)
    \item Il numero di proprietà inverse è bilanciato tra espressività e overhead computazionale
\end{itemize}

Queste scelte rendono l'ontologia \textbf{scalabile} e utilizzabile anche con dataset di dimensioni considerevoli (centinaia di ricette, migliaia di ingredienti).
% -----------------------------------------------------
...
Se nell'ontologia fossero definite classi equivalenti basate su restrizioni (ad esempio, \texttt{StrongBeer} definita come birra con ABV $\geq$ 7.0), il reasoner classificherebbe automaticamente le ricette nelle appropriate categorie senza bisogno di asserzioni esplicite.
